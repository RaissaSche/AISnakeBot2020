...
public class GameLogic : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < NumberSnakes; i++)
        {
            ...
            Vector2 randomPosition;//aqui
            if (i == selectedId)
            {
                randomPosition = new Vector2(0.0f,0.0f);//aqui
            }
            else
            {
                randomPosition = new Vector2(//aqui
                Random.Range(
                   minX, maxX
                ),
                Random.Range(
    ...
    IEnumerator SpawnXOrbsEveryYSeconds(float timeBetweenSpawns)
    {
        ...
        {
            Vector2 randomOrbPos = new Vector2(Random.Range(minX, maxX), Random.Range(minY, maxY));//aqui
            GameObject newOrb = Instantiate(orbPreFab, randomOrbPos, Quaternion.identity);
            GameObject orbParent = GameObject.Find("Orbs");
            newOrb.transform.parent = orbParent.transform;//aqui
            newOrb.transform.name = "Orb "+ count;//aqui
            count++;//aqui
        }
